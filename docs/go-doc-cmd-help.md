If a package is part of the go standard library or it is part of an installed module the following instructions are for the `go doc` command which will print out documentation about a specific package.
...
Given no arguments, that is, when run as `go doc` it prints the package documentation for the package in the current directory. If the package is a command (package main), the exported symbols of the package are elided from the presentation unless the -cmd flag is provided.
...
When run with two arguments, the first is a package path (full path or suffix), and the second is a symbol, or symbol with method or struct field: `go doc <pkg> <sym>[.<methodOrField>]`
...
- Show documentation for current package: `go doc`
- Serve HTML documentation over HTTP for the current package: `go doc -http`
- Show documentation for Foo in the current package: `go doc Foo` (Foo starts with a capital letter so it cannot match a package path.)
- Show documentation for the encoding/json package: `go doc encoding/json`
- Shorthand for encoding/json: `go doc json`
- Show documentation and method summary for json.Number: `go doc json.Number` (or `go doc json.number`)
- Show documentation for json.Number's Int64 method: `go doc json.Number.Int64` (or `go doc json.number.int64`)
- Show package docs for the doc command: `go doc cmd/doc`
- Show package docs and exported symbols within the doc command: `go doc -cmd cmd/doc`
- Show documentation for html/template's New function: `go doc template.new` (`html/template` is lexically before `text/template`)
- Show documentation for text/template's New function: `go doc text/template.new` # One argument
- Show documentation for text/template's New function: `go doc text/template new` # Two arguments
...
- `-src` Show the full source code for the symbol. This will display the full Go source of its declaration and definition, such as a function definition (including the body), type declaration or enclosing const block. The output may therefore include unexported details.
